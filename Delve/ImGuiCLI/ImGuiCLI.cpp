...
#define LBL ToSTLString(label).c_str()
    ...
    bool ImGuiCli::BeginPopupModal(System::String^ title, ImGuiWindowFlags_ windowFlags)
    {
        return ImGui::BeginPopupModal(ToSTLString(title).c_str(), 0x0, (int)windowFlags);
    }
    ...
    void ImGuiEx::AddLine(Vector2 a, Vector2 b, Color color, float thickness)
    {
        ImGui::GetWindowDrawList()->AddLine(ImVec2(a.X, a.Y), ImVec2(b.X, b.Y), color.PackedValue, thickness);
    }
}
...
Vector2 ImGuiCLI::ImGuiIO::MouseDelta::get() { return Vector2(ImGui::GetIO().MouseDelta.x, ImGui::GetIO().MouseDelta.y); }
...
float ImGuiCLI::ImGuiIO::CurrentDPI::get()
{
    if (auto win = ImGui::GetCurrentWindow())
    {
        if (auto vp = win->Viewport)
            return vp->DpiScale;
    }
    return 1.0f;
}
