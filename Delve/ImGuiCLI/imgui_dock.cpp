...
struct DockContext
{
	...
	Dock& getDock(const char* label, bool opened, ImGuiDockFlags flags)
	{
		...
		for (int i = 0; i < m_docks.size(); ++i)
		{
            ...
            {
                if (m_docks[i]->hidden)
                    m_docks[i]->wasHidden = true;
                m_docks[i]->hidden = flags & ImGuiDockFlags_Hidden;
                m_docks[i]->noCloseButton = flags & ImGuiDockFlags_NoCloseButton;
                return *m_docks[i];
            }
		}
        ...
        new_dock->hidden = flags & ImGuiDockFlags_Hidden;
        new_dock->noCloseButton = flags & ImGuiDockFlags_NoCloseButton;
		new_dock->invalid_frames = 0;
		new_dock->location[0] = 0;
        if (flags & (ImGuiDockFlags_StartLeft | ImGuiDockFlags_StartRight | ImGuiDockFlags_StartTop | ImGuiDockFlags_StartBottom))
            new_dock->status = Status_Docked;
        if (flags & ImGuiDockFlags_StartLeft)
            new_dock->location[0] = '1';
        else if (flags & ImGuiDockFlags_StartRight)
            new_dock->location[0] = '2';
        else if (flags & ImGuiDockFlags_StartTop)
            new_dock->location[0] = '3';
        else if (flags & ImGuiDockFlags_StartBottom)
            new_dock->location[0] = '4';
		return *new_dock;
	}
...
#define SPLIT_SIZE 6
	void splits()
	{
        ...
        ImColor src(color);
        ImU32 colorFaint = ImColor(src.Value.x, src.Value.y, src.Value.z, 0.1f);
		...
		for (int i = 0; i < m_docks.size(); ++i)
		{
			...
			{
				InvisibleButton("split", ImVec2(SPLIT_SIZE, dock.size.y));
				if (dock.status == Status_Dragged) 
                    dsize.x = io.MouseDelta.x;
                if (IsItemHovered() && GetIO().MouseDoubleClicked[0])
                {
                    if (dock.parent && dock.parent->children[0] == &dock)
                        dsize.x = -100000;
			...
			{
				InvisibleButton("split", ImVec2(dock.size.x, SPLIT_SIZE));
				if (dock.status == Status_Dragged) 
            ...
            auto cursorType = dock.isHorizontal() ? ImGuiMouseCursor_ResizeEW : ImGuiMouseCursor_ResizeNS;
            if (IsItemHovered() && IsMouseClicked(0))
			{
                ImGui::SetMouseCursor(cursorType);
				dock.status = Status_Dragged;
			}
            else if (IsItemHovered() || dock.status == Status_Dragged)
            {
                ImGui::SetMouseCursor(cursorType);
            }

            // only draw when hovered or dragging
            if (IsItemHovered() || dock.status == Status_Dragged)
                ...
                draw_list->AddRectFilled(GetItemRectMin(), GetItemRectMax(), colorFaint);
			PopID();
		}
	}
...
#define SIZE_BIG 100
	...
	static ImRect getSlotRect(ImRect parent_rect, Slot_ dock_slot)
	{
		...
		switch (dock_slot)
		{
			default: return ImRect(center - ImVec2(SIZE_SMALL, SIZE_SMALL), center + ImVec2(SIZE_SMALL, SIZE_SMALL));
			case Slot_Top: return ImRect(center + ImVec2(-SIZE_SMALL, -SIZE_BIG), center + ImVec2(SIZE_SMALL, -SIZE_MID));
			case Slot_Right: return ImRect(center + ImVec2(SIZE_MID, -SIZE_SMALL), center + ImVec2(SIZE_BIG, SIZE_SMALL));
			case Slot_Bottom: return ImRect(center + ImVec2(-SIZE_SMALL, +SIZE_MID), center + ImVec2(SIZE_SMALL, SIZE_BIG));
			case Slot_Left: return ImRect(center + ImVec2(-SIZE_BIG, -SIZE_SMALL), center + ImVec2(-SIZE_MID, SIZE_SMALL));
		}
	}
	...
	static ImRect getSlotRectOnBorder(ImRect parent_rect, Slot_ dock_slot)
	{
		...
		{
			case Slot_Top:
				return ImRect(ImVec2(center.x - SIZE_SMALL, parent_rect.Min.y + SIZE_TINY),
					ImVec2(center.x + SIZE_SMALL, parent_rect.Min.y + SIZE_MID));
			case Slot_Left:
				return ImRect(ImVec2(parent_rect.Min.x + SIZE_TINY, center.y - SIZE_SMALL),
					ImVec2(parent_rect.Min.x + SIZE_MID, center.y + SIZE_SMALL));
			case Slot_Bottom:
				return ImRect(ImVec2(center.x - SIZE_SMALL, parent_rect.Max.y - SIZE_MID),
					ImVec2(center.x + SIZE_SMALL, parent_rect.Max.y - SIZE_TINY));
			case Slot_Right:
				return ImRect(ImVec2(parent_rect.Max.x - SIZE_MID, center.y - SIZE_SMALL),
					ImVec2(parent_rect.Max.x - SIZE_TINY, center.y + SIZE_SMALL));
	...
	bool dockSlots(Dock& dock, Dock* dest_dock, const ImRect& rect, bool on_border)
	{
		...
		{
            if (((dest_dock && dest_dock->noTabs) || dock.noTabs) && i == Slot_Tab)
                continue;
			ImRect r =
				on_border ? getSlotRectOnBorder(rect, (Slot_)i) : getSlotRect(rect, (Slot_)i);
			bool hovered = r.Contains(mouse_pos);
			
			canvas->AddRectFilled(r.Min, r.Max, hovered ? color_hovered : color, 4);
            canvas->AddRect(r.Min, r.Max, color_hovered, 4, 15, 3);
			if (!hovered) continue;
	...
	bool tabbar(Dock& dock, bool close_button)
	{
        ...
        while (dock_tab)
        {
            anyVisible |= (dock_tab->hidden || dock_tab->opened);
		...
		if (BeginChild(tmp, size, true))
		{
			...
			while (dock_tab)
			{
				...
				ImVec2 size(CalcTextSize(dock_tab->label, text_end).x + 10, line_height);
				...
				draw_list->AddRectFilled(pos+ImVec2(-8.0f, 0.0),
										 ...
										 hovered ? 
                                            color_hovered : 
                                            (dock_tab->active ? color_active : color), 6.0f, ImDrawCornerFlags_Top);
				draw_list->AddText(pos + ImVec2(0, 2), text_color, dock_tab->label, text_end);
	...
	bool begin(const char* label, bool* opened, ImGuiWindowFlags extra_flags, ImGuiDockFlags dockFlags)
	{
		Dock& dock = getDock(label, !opened || *opened, dockFlags);
        if (dockFlags & ImGuiDockFlags_NoTabs)
            dock.noTabs = true;
        if (dockFlags & ImGuiDockFlags_NoPad)
            dock.noPad = true;
		...
		float tabbar_height = GetTextLineHeightWithSpacing() + 10;
        bool drewTabs = true;
        if (dock.noTabs)
        {
            drewTabs = false;
		...
		else if (tabbar(dock.getFirstTab(), opened != nullptr))
		{
			fillLocation(dock);
			*opened = false;
		}
        ...
        float yAdjust = extra_flags & ImGuiWindowFlags_MenuBar ? 12 : 6;
        pos.y += yAdjust;
        size.y -= yAdjust;
        pos.x +=  !dock.noPad ? SPLIT_SIZE : 0;
	...
	void load()
	{
		...
		if (fp) {
			...
			for (int i = 0; i < ival; i++) {
                ...
                fscanf(fp, "%s %u", str2, &noTabs);
                fscanf(fp, "%s %u", str2, &noPad);
                ...
                m_docks[id]->noTabs = noTabs == 1 ? true : false;
                m_docks[id]->noPad = noPad == 1 ? true : false;
...
bool BeginDock(const char* label, bool* opened, ImGuiWindowFlags extra_flags, ImGuiDockFlags flags)
{
	return g_dock.begin(label, opened, extra_flags, flags);
}
