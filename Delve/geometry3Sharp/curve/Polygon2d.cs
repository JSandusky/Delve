...
namespace g3
{
	public class Polygon2d : IDuplicatable<Polygon2d>
    {
        ...
        public void Shrink(double shrinkDist)
        {
            List<Vector2d> NewV = new List<Vector2d>();
            int N = Vertices.Count;

            List<Vector2d> norms = new List<Vector2d>();
            for (int i = 0; i < N; ++i)
                norms.Add(GetNormal_FaceAvg(i));

            for (int i = 0; i < N; ++i)
                vertices[i] = vertices[i] + norms[i] * shrinkDist;

            Timestamp++;
        }

        public void ShrinkByLines(double shrinkDist)
        {
            ...
            for (int i = 0; i < Vertices.Count; ++i)
            {
                ...
                Line2d prevLine = Line2d.FromPoints(this[prev], this[i]);
                Line2d nextLine = Line2d.FromPoints(this[i], this[next]);
                IntrLine2Line2 test = new IntrLine2Line2(prevLine, nextLine);
                test.Compute();
