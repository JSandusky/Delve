...
{
    public static class DragObjectHelper
    {
        static object dragging_;
    ...
    public class GenericObjectTree
    {
        ...
        protected virtual void DrawNodeObject(object obj, string text, bool isSelected)
        {
            ...
            if (DragConverter != null)
            {
                ...
                {
                    ImGuiCli.SetDragDropPayload("U_OBJREF", DragConverter(obj));
                    DragObjectHelper.Dragging = obj;
                    ImGuiCli.Text(text);
        ...
        void Draw(IList list, int depth)
        {
            for (int i = 0; i < list.Count; ++i)
            {
                ...
                ImGuiCli.PushID(id);
                object obj = list[i];
                var listFields = cache_.GetAlphabetical(obj.GetType()).ForEditor(obj.GetType(), "GenericObjectTree").Where(f => typeof(IList).IsAssignableFrom(f.Type)).ToArray();
