...
PSOut PSBasic(VSOutput pin)
{
    ...
    PSOut ret;
    ret.Color = color;
    ret.Depth = pin.PositionPS.z;
    return ret;
}


// Pixel shader: no fog.
PSOut PSBasicNoFog(VSOutputNoFog pin)
{
    PSOut ret;
    ret.Color = pin.Diffuse;
    ret.Depth = pin.PositionPS.z;
    return ret;
}
...
PSOut PSBasicVertexLighting(VSOutput pin)
{
    float4 color = pin.Diffuse;
...
PSOut PSBasicVertexLightingNoFog(VSOutput pin)
{
    float4 color = pin.Diffuse;
    ...
    PSOut ret;
    ret.Color = color;
    float d = (pin.PositionPS.z/pin.PositionPS.w)*0.5+0.5;
    ret.Depth = float4(d,d,d,1); //(mul(pin.PositionPS, WorldViewProj).z / -200.0);
    //ret.Depth = pin.PositionPS.w;
    return ret;
}


// Pixel shader: vertex lighting + texture.
PSOut PSBasicVertexLightingTx(VSOutputTx pin)
{
    float4 color = SAMPLE_TEXTURE(Texture, pin.TexCoord) * pin.Diffuse;
...
PSOut PSBasicVertexLightingTxNoFog(VSOutputTx pin)
{
    float4 color = SAMPLE_TEXTURE(Texture, pin.TexCoord) * pin.Diffuse;
...
PSOut PSBasicPixelLighting(VSOutputPixelLighting pin)
{
    float4 color = pin.Diffuse;
...
PSOut PSBasicPixelLightingTx(VSOutputPixelLightingTx pin)
{
    float4 color = SAMPLE_TEXTURE(Texture, pin.TexCoord) * pin.Diffuse;
